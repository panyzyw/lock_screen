...
public class LockscreenUtils {
	...
	public void unlock() {
		if (Contant.DEBUG)
			Contant.showTips("LockscreenUtils"+ "____" + "unlock");
		if (mOverlayDialog != null) {
			mOverlayDialog.dismiss();
			mOverlayDialog = null;
			if(mLockStatusChangedListener!=null)
			{
				Contant.showTips("LockscreenUtils"+ "____" + "onLockStatusChanged");
	...
	private static class OverlayDialog extends AlertDialog {
		...
		public OverlayDialog(Activity activity) {
			...
			try{
				...
				params.type = LayoutParams.TYPE_SYSTEM_OVERLAY;//解决表情遮挡的问题
				params.dimAmount = 0.0F;
				params.width = 0;
				params.height = 0;
				params.gravity = Gravity.BOTTOM;
				getWindow().setAttributes(params);
				getWindow().setFlags(LayoutParams.FLAG_SHOW_WHEN_LOCKED | LayoutParams.FLAG_NOT_TOUCH_MODAL|LayoutParams.SOFT_INPUT_STATE_HIDDEN,
						0xffffff);
				setOwnerActivity(activity);
				setCancelable(false);
			}catch(Exception e){
				Contant.showTips("LockscreenUtils"+ "_Exception___" + "OverlayDialog"+e.getMessage());
			}
		}
